...
input_required_details() {
    ...
    read -p "Enter the token name (e.g., HiddenGem Token): " TOKEN_NAME
    read -p "Enter the token symbol (e.g., HiddenGem): " TOKEN_SYMBOL
...
deploy_contract() {
    ...
    cat <<EOL > "$SCRIPT_DIR/src/HiddenGem.sol"
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract HiddenGem is ERC20 {
...
EOL
    ...
    DEPLOY_OUTPUT=$(forge create "$SCRIPT_DIR/src/HiddenGem.sol:HiddenGem" \
        --rpc-url rpc_url \
